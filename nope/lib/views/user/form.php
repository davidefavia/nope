  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <div class="form-group" ng-if="!user.id" ng-class="{'has-error':(!userForm.username.$valid && userForm.username.$touched)}">
        ...
        <input type="text" name="username" class="form-control" ng-model="user.username" required   ng-pattern='<?php echo \Nope\Utils::USERNAME_REGEX_PATTERN; ?>' ng-trim="false" placeholder="Username" />
      ...
      <div class="form-group" ng-class="{'has-error':((!userForm.password.$valid && userForm.password.$touched) || (!userForm.confirm.$valid && userForm.confirm.$touched))}">
        <div class="row">
          <div class="col col-md-6">
              ...
              <input type="password" name="password" class="form-control" ng-model="user.password" ng-required="!user.id" placeholder="Password" />
          ...
          <div class="col col-md-6">
              ...
              <input type="password" name="confirm" class="form-control" ng-model="user.confirm" ng-required="!user.id" nope-match="user.password" placeholder="Confirm password" />
      ...
      <div class="form-group" ng-class="{'has-error':(!userForm.email.$valid && userForm.email.$touched)}">
        ...
        <input type="text" ng-pattern='<?php echo \Nope\Utils::EMAIL_REGEX_PATTERN; ?>' name="email" class="form-control" ng-model="user.email" required placeholder="Email to reset password" />
      ...
      <div class="form-group">
        ...
        <input type="text" name="prettyname" class="form-control" ng-model="user.prettyName"  placeholder="Pretty name to display" />
      </div>
      <div class="checkbox">
        <label>
      ...
      <div class="form-group">
        ...
        <textarea name="description" class="form-control" ng-model="user.description" placeholder="User brief description" rows="5"></textarea>
      </div>
      <div class="form-group">
        <label>Cover</label>
        <nope-model href="#/media?mimetype=image/" ng-model="user.cover" multiple="false" label="Add cover" preview="icon" template="media"></nope-model>
      </div>
    </div>
