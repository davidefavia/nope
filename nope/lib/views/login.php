  ...
  <div class="row flex-items-xs-center">
    <div class="col-md-4">
      <div class="text-xs-center">
        <h1><img ng-src="{{assetsPath + 'assets/img/nope.png'}}" /></h1>
      </div>
      <div class="card" ng-hide="recovery || useResetCode">
        <div class="card-block">
          <form class="form-signin" name="loginForm" ng-submit="login()">
            ...
            <div class="form-group clearfix">
              <a ng-click="recovery=true" class="btn btn-link">Forgot password</a>
              <button class="btn pull-right" type="submit" ng-class="{'btn-success':!loginForm.$invalid}" ng-disabled="loginForm.$invalid">Sign in</button>
            </div>
          </form>
        </div>
      </div>
      <div class="card" ng-show="recovery">
        <div class="card-block">
          <form class="form-signin" ng-show="!recoveryStatus" name="recoveryForm" ng-submit="recoveryPassword()">
            <div class="form-group" ng-class="{'has-error':(!recoveryForm.email.$valid && recoveryForm.email.$touched)}">
              ...
              <input name="email" class="form-control input-lg" placeholder="Email" required type="email" ng-pattern='<?php echo \Nope\Utils::EMAIL_REGEX_PATTERN; ?>' ng-model="recoveryEmail" />
            ...
            <div class="form-group clearfix">
              <a ng-click="recovery=false;recoveryStatus=false" class="btn btn-link">&laquo; Go to login</a>
              <button class="btn pull-right" type="submit" ng-class="{'btn-success':!recoveryForm.$invalid}" ng-disabled="recoveryForm.$invalid">Send</button>
            </div>
          </form>
      ...
      <div class="card" ng-show="useResetCode">
        <div class="card-block">
          <form class="form-signin" name="resetPasswordForm" ng-submit="resetPassword()">
            ...
            <div class="form-group">
              <button type="submit" class="btn btn-success btn-block" ng-disabled="resetPasswordForm.$invalid">Reset password and login</button>
            </div>
          </form>
        </div>
      </div>
      <h6 class="text-xs-center">v<?php echo NOPE_VERSION; ?></h6>
    </div>
  </div>
</div>
